 ...
 * Handles trailing separators on both folder and cwd.
 */
function isSessionInFolder(cwd: string, folder: string): boolean {
  ...
  // Exact match
  if (normalizedCwd === normalizedFolder) {
    return true
  }

  // Check if cwd is in a subdirectory of folder
  const relPath = relative(normalizedFolder, normalizedCwd)
  ...
  // so cwd is in a subdirectory of folder (or we already handled exact match above)
  return true
}
